...
% few words on recent heat transfer modelling study: rajah et el
...
%  \citep{Carrivick&al2020}  gives good state of the art on what's missing in glacier-lake interaction numericasl modelling (but not necessarily GLOFs!), like data to validate models
...
%cite Gleason 2016 (Greenland) on the need of characterizing the flow resistance for surface draiange
...
\subsection{Detecting and mapping glacial lakes before the outburst}% that is where geophyiscal investigations and in particular GPR fits in
\todo{The aim of this section is to introduce the challenges in detection of water pockets, and ultimately to the limitation of the GPR, whi is the third chapter if the thesis. That said, I admit that this section looks a bit orphan as it is}
...
\todo{cite some of James Irving's work}

%% MUCH MORE ON GPR NEEDED!
...
% observed trend and progress % for now not implemented
...
\hline
%- Empirical parametrization of heat transfer from water to ice from engineering literature is inappropriate for jökulhlaups {\citep{Roberts2005,Sommers&Rajaram2020}} \\
...
% \begin{table}[h]
% \centering
% \caption{Summary of the research gaps on GLOFs and proposed ways forward identified in this Introduction.}
% \begin{tabular}{l}
% \hline
% \textbf{Identified research gaps in GLOFs research}  \\
% \hline
% \textit{GLOFs observation and monitoring} \\
% \hline
% - Deficiencies in GLOFs understanding due to lack of pertinent, high-quality observational data of GLOFs \\{\citep{Roberts2005,Huss&al2007}} \\
% - Need for dataset on relevant parameters required by GLOFs model (e.g. breach parameters, lake bathymetry,\\ lake outflow discharge), as well as quantifying their uncertainties {\citep{Emmer&al2022,Zhang&al2024}}\\
% - Numerical modelling of GLOFs is challenged by absent knowledge of parameter values such as\\ lake water temperature or time-varying lake level, limiting insight {\citep{Carrivick&al2020}}\\
% - Hydrodynamic modelling and reconstruction of GLOFs is hindered by absent data {\citep{Zhang&al2024}}\\
% %-  This study highlights the need for an integrative assessment of glacier-dammed lake floods to better understand these events \citep{Huss&al2007}
...
% %- Understanding of GLOFs is severely limited by inadequate data. Triggers are often unknown or speculated owing to absent field data, necessitating reliance on climatic or geological–geomorphological evidence [35,76–78] \citep{Zhang&al2024}
...
% \textit{Englacial and sublgacial outburst floods}   \\
% \hline
% - lack of consensus on water pocket definition even in most recent studies\\  {\citep[e.g. in][]{Lutzow&al2023,Zhang&al2024}}\\
% - Proper terminology should be maintained among researchers, disaster risk reduction practitioners,\\ and authorities to avoid misinterpretation of individual events {\citep{Emmer&al2022}}\\
% - More comprehensive glacial outburst floods inventories are essential for better understanding frequency of\\ GLOFs occurrence in changing mountain environments and for revealing frequency–magnitude relationships {\citep{Veh&al2022,Emmer&al2022}}\\
% - Defining realistic GLOFs scenarios is crucial in predictive GLOFs modelling {\citep{Emmer&al2022,Zhang&al2024}},\\ but there is for now too little knowledge on water pocket rupture scenario to do so {\citep{Haeberli1983}}\\
% - Need more observation of englacial water reservoir {\citep{Haeberli1983}}\\
% - The use of ground penetrating radar for detecting englacial water bodies in temperate ice \\is controversial and requires more theoretical considerations \\

% \textit{Hydraulics and thermodynamics}\\
% \hline
% - A viable description of jökulhlaup mechanisms demands more research into the rate of heat transfer from flood water to the surrounding ice {\citep{Bjornsson2010}}\\
% %- Empirical parametrization of heat transfer from water to ice from engineering literature are inadequate for the problems posed by jökulhlaups {\citep{Roberts2005}} \\
% - Need for quantitative analysis of heat transfer between water and ice, i.e. the relationship between Nusselt number, Reynolds number and Prandtl number {\citep{Clarke2003,Roberts2005}} \\
% %- Lack of independent parameterization for heat transfer \cite{Vincent&al2010} and friction factor \cite{Werder&al2010}, preventing independent modeling \\
% -Lack of in situ measurements on the flow resistance at the ice interface during ice-dammed lake drainage {\citep{Clarke2003,Werder&al2010,Kingslake&al2015,Gleason&al2016}}\\
% - The flow resistance is poorly constrained from field observations and lead to large uncertainties in modelled GLOFs peak discharge {\citep{Werder&al2010}}\\

% \hline
% \textbf{Ways forward (this thesis)}  \\
% \hline
% - To provide an extensive data set of an ice-dammed lake drainage for further validation of GLOFs modelling studies (Section~\ref{ch:chapter_plainemorte})\\
% - To provide value on stream flow resistance in ice channels (and their uncertainties)(Section~\ref{ch:chapter_plainemorte})\\
% - To provide a relationship between Nusselt number, Reynolds number and Prandtl number on field experiment to quantify the heat transfer at the water/ice interface (Section~\ref{ch:chapter_plainemorte})\\
% - To provide an up-to-date and comprehensive inventory of water pocket outburst flood in the Swiss Alps (Section~\ref{ch:chapter_WPOFs})\\
% - To characterize the drivers for the formation and the rupture of water pockets (Section~\ref{ch:chapter_WPOFs})\\
% - To propose mechanisms of formation and rupture of water pockets (Section~\ref{ch:chapter_WPOFs})\\
% - To evaluate the limitations of ground penetrating radar in temperate ice for the detection of englacial water bodies (Section~\ref{ch:chapter_gprmax})\\
